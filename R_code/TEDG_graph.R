...
for(patient in 1:100){
  ...
  #infilepath <- paste0("~/Desktop/FINAL_FYT/region_num/mut_table/",infile)
  ...
  if(is.na(muttable["countfreq"]) == FALSE){
    ...
    #### Determine time line of the mutation with difference threshold 5
    if(length(temp) > 1){
      for(x in 2:nrow(muttable.sort)-1){
        for(y in (x+1):nrow(muttable.sort)){
          if(((muttable.sort[x,"total"] - muttable.sort[y, "total"]) > 5) ){
...
write.table(edge.table,"TEDGedge.original.txt",row.names = F, sep = '\t')
write.table(node.table,"TEDGnode.txt",row.names = F, sep = '\t')
...
write.table(edge[which(as.numeric(edge$weight) > 2),],"TEDGedge.modified.txt",row.names = F, sep = '\t')
...
#pdf(file="Figure4_TEDG.pdf", bg = 'white', width = 8, height = 6)
plot(net)
dev.off()
plot(net)

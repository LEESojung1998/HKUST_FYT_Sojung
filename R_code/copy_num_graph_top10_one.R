...
#### based on gene
gene.counted <- read.delim("~/Desktop/FINAL_FYT/copy_num/gene.counted.txt")
gene.counted <- gene.counted[order(gene.counted$frequency, decreasing = TRUE),]
gene.top12 <- gene.counted$gene.list[1:12]

#candidates.new <- read.delim("~/Desktop/FINAL_FYT/copy_num/candidates.new.txt", header=FALSE)
gene.list <- gene.top12
# 
...
figure_2 <- rbind(ggplotGrob(plot1),ggplotGrob(plot2),ggplotGrob(plot3),ggplotGrob(plot4),ggplotGrob(plot5),ggplotGrob(plot6),ggplotGrob(plot7),ggplotGrob(plot8),ggplotGrob(plot9),ggplotGrob(plot10),ggplotGrob(plot11),ggplotGrob(plot12),size="last")
ggsave(file="~/Desktop/FINAL_FYT/copy_num/copynum.matrix/final_top10_graph.pdf", plot=figure_2,bg = 'white', width = 120, height = 60, units = 'cm', dpi = 600)


#### top 9
########### divide by gene

gene.top9 <- gene.counted$gene.list[1:9]
...
for(gene in gene.list){
  ...
  if(count == 9){
    ...
    plot.name<-plot.name+geom_tile(data = plot.data,aes(x=Var1,y=Var2,fill=value),color='black',width=0.7,height=0.7,stat='identity')
    plot.name<-plot.name+scale_fill_manual(name=NULL,values=c(amp = gg_color_hue(4)[4],normal = 'white', loss = gg_color_hue(4)[3], deletion = gg_color_hue(4)[2], gain = gg_color_hue(4)[1]),labels=c(gain = "gain", loss = "loss", deletion = "deletion", amp = "amplification", normal = "normal"))
    plot.name<-plot.name+theme(panel.background=element_rect(fill='transparent',color='black'),plot.margin=unit(c(0.15,1,1,2),'lines'),plot.title=element_text(size=30,vjust=0.5,hjust=0.5,face='bold.italic'),
                               text=element_text(size=24,vjust=1.4,hjust=0.5,face='bold'),legend.key.width=unit(0.6,'cm'),legend.key.height=unit(0.68,'cm'),legend.position='right',
                               legend.direction="horizontal",legend.text=element_text(size=10,face='bold.italic'),axis.text.y=element_text(size=28,vjust=0.5,hjust=1,face='bold.italic',color='black'),
                               axis.text.x=element_text(size=8,angle=90,vjust=0.5,hjust=0,face='bold',color='black'),axis.title.x=element_text(size=35,vjust=0,hjust=0.5,face='bold',color='black'),axis.title.y=element_text(size=35,hjust=0.5,vjust=2,face='bold',color='black'))
    plot.name<-plot.name+scale_x_discrete(breaks=x.scale,position = "bottom")+xlab(NULL)+ylab(NULL)+ggtitle(gene)
    ...
    plot.name<-plot.name+theme(panel.background=element_rect(fill='transparent',color='black'),plot.margin=unit(c(0.15,1,1,2),'lines'),plot.title=element_text(size=24,vjust=0.5,hjust=0.5,face='bold.italic'),
                               text=element_text(size=24,vjust=1.4,hjust=0.5,face='bold'),legend.key.width=unit(0.6,'cm'),legend.key.height=unit(0.68,'cm'),legend.position='right',
                               legend.direction="horizontal",legend.text=element_blank(),axis.text.y=element_text(size=28,vjust=0.5,hjust=1,face='bold.italic',color='black'),
                               axis.text.x=element_blank(),axis.title.x=element_text(size=24,vjust=0,hjust=0.5,face='bold',color='black'),axis.title.y=element_text(size=35,hjust=0.5,vjust=2,face='bold',color='black'))
    plot.name<-plot.name+scale_x_discrete(breaks=x.scale,position = "top")+xlab(NULL)+ylab(NULL)+ggtitle(gene)
    # outfile2 <- paste0("~/Desktop/FINAL_FYT/copy_num/copynum.matrix/",paste0(gene,"_copynum.Matrix.pdf"))
    # ggsave(file=outfile2, plot=plot.name,bg = 'white', width = 70, height = 5, units = 'cm', dpi = 600)
    if(count == 2){
...
figure_2 <- rbind(ggplotGrob(plot1),ggplotGrob(plot2),ggplotGrob(plot3),ggplotGrob(plot4),ggplotGrob(plot5),ggplotGrob(plot6),ggplotGrob(plot7),ggplotGrob(plot8),ggplotGrob(plot9),size="last")
ggsave(file="~/Desktop/FINAL_FYT/copy_num/copynum.matrix/final_top9_graph.pdf", plot=figure_2,bg = 'white', width = 120, height = 60, units = 'cm', dpi = 600)
